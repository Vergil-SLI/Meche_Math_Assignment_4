...
function usage_example()
    ...
    n_samples = 30;
    h_ref_list = logspace(-6,1, 30);
    
    num_evals_list = zeros(1, n_samples);
    h_avg_list = zeros(1, n_samples);
    tr_error_list = zeros(1, n_samples);
    V_list = compute_planetary_motion(t_range,V0,orbit_params)';
    local_tr_error_list = zeros(1, n_samples);
    analytical_difference = zeros(1, n_samples);
    ...
    for i = 1:length(h_ref_list)
        ...
        [XB, num_evals] = explicit_RK_step(rate_func_in,t_ref,V0,h_ref,BT_struct);
        x_analytical = compute_planetary_motion(h_ref_list(i), V0, orbit_params);
        analytical_difference(i) = norm(x_analytical - V0);
        local_tr_error_list(i) = norm(XB - x_analytical);
    end
 ...
 % create log fit
    ...
    [localp1,localk1] = loglog_fit(h_ref_list,local_tr_error_list, filter_params);
    localp1
    ...
    hold on
    loglog(h_ref_list, localk1*h_ref_list.^localp1, 'k-', 'markerfacecolor', 'r');
    hold on
    loglog(h_ref_list, analytical_difference, 'r-', 'markerfacecolor', 'r');
    legend("Heun's Method", "Fit Line", "Analytical Difference")
    title("Local Truncation error of Heun's method vs step size")
    xlabel("Step size h")
    ylabel("Error")
    hold off
